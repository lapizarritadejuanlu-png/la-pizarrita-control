...
function taxMonthlyLoad(x){const a=Number(x.amount)||0,c=x.recurrence||'oneoff';if(c==='quarterly')return a/3;if(c==='annual')return a/12;return a}
...
function expenseTotals(rows=monthExpenseRows()){
  let operating=0,taxCash=0,taxLoad=0,investment=0;
  for(const x of rows){const a=Number(x.amount)||0,k=expenseKind(x);if(k==='investment')investment+=a;else if(k==='tax'){taxCash+=a;taxLoad+=taxMonthlyLoad(x)}else operating+=a}
...
function expensesForMonthHtml(){
  const rows=monthExpenseRows(),t=expenseTotals(rows),cats=expenseCategorySummary(rows);
...
function enhancedDashboard(){
  const inv=monthRows(invoices,'invoice_date'),mv=monthRows(moves,'move_date'),sales=mv.filter(x=>x.move_type==='ingreso').reduce((a,x)=>a+Number(x.amount||0),0),purchases=inv.reduce((a,x)=>a+Number(x.total||0),0),personal=mv.filter(x=>x.move_type==='personal').reduce((a,x)=>a+Number(x.amount||0),0),expenses=mv.filter(x=>x.move_type==='gasto'),t=expenseTotals(expenses),result=sales-purchases-personal-t.operating-t.taxLoad,cashOut=purchases+personal+t.cash;
