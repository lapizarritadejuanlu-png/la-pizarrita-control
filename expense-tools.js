...
function enhancedDashboard(){
  const inv=monthRows(invoices,'invoice_date'),mv=monthRows(moves,'move_date'),sales=mv.filter(x=>x.move_type==='ingreso').reduce((a,x)=>a+Number(x.amount||0),0),purchases=inv.reduce((a,x)=>a+Number(x.total||0),0),personal=mv.filter(x=>x.move_type==='personal').reduce((a,x)=>a+Number(x.amount||0),0),expenses=mv.filter(x=>x.move_type==='gasto'),t=expenseTotals(expenses,selectedMonth),result=sales-purchases-personal-t.operating-t.taxLoad,personalCash=window.personnelCashForMonth?window.personnelCashForMonth(selectedMonth):null,cashOut=personalCash===null?null:purchases+personalCash+t.cash;
  ...
  return `<div class="card dashboard-card"><div class="dash-label">BALANCE DE CAJA DEL MES · ${esc(selectedMonth)}</div><div class="big-result" style="color:${cashOut===null?'var(--muted)':sales-cashOut<0?'#ff9a91':'var(--mint)'}">${cashOut===null?'No disponible':euro(sales-cashOut)}</div><p class="cash-note">Entradas ${euro(sales)} − salidas ${cashOut===null?'no disponibles':euro(cashOut)}. Incluye los pagos de nóminas registrados este mes.</p><div style="margin:14px 0;padding:12px;border:1px solid var(--line);border-radius:12px"><div style="font-size:1rem;color:var(--muted)">Resultado operativo estimado del mes</div><strong style="font-size:1.2rem;color:${result<0?'#ff9a91':'var(--mint)'}">${euro(result)}</strong></div><select id="monthPicker" class="month-select">${opts}</select><div class="metrics"><div class="metric"><span>💵 Ventas</span><strong>${euro(sales)}</strong></div><div class="metric"><span>🛒 Compras</span><strong>${euro(purchases)}</strong></div><div class="metric"><span>👥 Coste de personal</span><strong>${euro(personal)}</strong></div><div class="metric"><span>🧾 Gasto operativo</span><strong>${euro(t.operating)}</strong></div><div class="metric"><span>🏛️ Impuestos (carga)</span><strong>${euro(t.taxLoad)}</strong><div class="dash-tax-sub">Pagado: ${euro(t.taxCash)}</div></div><div class="metric"><span>🏗️ Inversiones</span><strong>${euro(t.investment)}</strong></div></div><p class="cash-note">Salida de caja estimada del mes: <strong>${cashOut===null?'No disponible':euro(cashOut)}</strong>. Personal en caja: ${personalCash===null?'no disponible':euro(personalCash)} (pagos registrados y horas extra). El coste de las nóminas cuenta en el mes trabajado; el pago, en su fecha. Las inversiones cuentan en caja, pero no reducen el resultado operativo.</p>${cats.length?`<div class="dash-expense-breakdown">${cats.map(([c,a])=>`<div class="expense-cat-row"><span>${esc(expenseLabel(c))}</span><strong>${euro(a)}</strong></div>`).join('')}</div>`:''}</div><div class="section-title">Últimas facturas</div>${recent.length?`<div class="list">${recent.map(x=>invoiceRow(x,false)).join('')}</div>`:'<div class="empty">Todavía no hay facturas.</div>'}`
}
