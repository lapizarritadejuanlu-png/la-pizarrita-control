(()=>{
let accountingMonthLocks=[],accountingMonthCloseHistory=[];
...
function monthSnapshot(m){
  ...
  return{document_count:docs.length,accounting_count:accounting.length,invoice_count:formal.length,ticket_count:accounting.filter(x=>x.document_type==='ticket').length,supplier_count:new Set(accounting.map(x=>String(x.supplier||'').trim().toLowerCase()).filter(Boolean)).size,net_total:sum(accounting,'total'),invoice_base:sum(formal,'base_amount'),invoice_vat:sum(formal,'vat_amount'),captured_at:new Date().toISOString()};
}
async function loadMonthLocks(){
  try{
    const [locks,history]=await Promise.all([
      api('/rest/v1/accounting_month_locks?select=month,locked_at,summary&order=month.desc'),
      api('/rest/v1/accounting_month_close_history?select=month,action,summary,created_at&order=created_at.desc&limit=100')
    ]);
    accountingMonthLocks=Array.isArray(locks)?locks:[];accountingMonthCloseHistory=Array.isArray(history)?history:[];window.accountingMonthLocks=accountingMonthLocks;window.accountingMonthCloseHistory=accountingMonthCloseHistory;
  }catch(e){console.warn('Month locks',e?.message||'unknown');accountingMonthLocks=[];accountingMonthCloseHistory=[]}
}
...
function snapshotGrid(summary){const s=summary||{};if(!s||typeof s!=='object'||s.accounting_count===undefined)return'';return `<div class="month-lock-snapshot"><div class="month-lock-stat"><span>Compras contabilizadas</span><strong>${Number(s.accounting_count)||0}</strong></div><div class="month-lock-stat"><span>Total neto</span><strong>${euro(Number(s.net_total)||0)}</strong></div><div class="month-lock-stat"><span>Base facturas</span><strong>${euro(Number(s.invoice_base)||0)}</strong></div><div class="month-lock-stat"><span>IVA facturas</span><strong>${euro(Number(s.invoice_vat)||0)}</strong></div></div>`}
function snapshotHtml(lock){return `${snapshotGrid(lock?.summary)}${lock?.locked_at?`<div class="month-lock-time">Cierre guardado: ${esc(closedAtText(lock.locked_at))}</div>`:''}`}
function previousClose(m){return accountingMonthCloseHistory.find(x=>String(x.month||'').slice(0,7)===m&&x.action==='closed')||null}
function previousCloseHtml(m){
  const x=previousClose(m);if(!x)return'';const s=x.summary||{};
  return `<div class="month-lock-previous"><div class="month-lock-previous-title">🕘 Último cierre anterior</div><div class="month-lock-previous-meta">${esc(closedAtText(x.created_at))} · ${Number(s.accounting_count)||0} compras · ${euro(Number(s.net_total)||0)} netos. Esta referencia se conserva aunque el mes haya sido reabierto.</div></div>`;
}
function monthLockCard(){
  const m=selectedMonth,lock=lockRecord(m),locked=!!lock,b=closeBlockers(m),blocked=!locked&&(b.pending.length||b.review.length),future=m>localDate().slice(0,7),historyCount=accountingMonthCloseHistory.filter(x=>String(x.month||'').slice(0,7)===m&&x.action==='closed').length;
  const cls=locked?' closed':blocked?' blocked':'';
  if(locked)return `<div class="section-title">Cierre mensual</div><div class="card month-lock-card closed"><div class="month-lock-head"><div class="month-lock-title">🔒 ${esc(monthLabel(m))}</div><div class="month-lock-state">CERRADO</div></div><div class="month-lock-help">Los documentos de este mes están protegidos contra altas, correcciones, vinculaciones, Papelera y borrado accidental. Las cifras siguientes son la foto exacta guardada al cerrar${historyCount>1?` · cierre nº ${historyCount}`:''}.</div>${snapshotHtml(lock)}<button type="button" id="reopenAccountingMonth" class="secondary month-lock-btn">🔓 Reabrir mes</button></div>`;
...
async function reopenAccountingMonth(){
  const m=selectedMonth;if(!confirm(`¿Reabrir ${monthLabel(m)}?\n\nVolverás a poder modificar sus documentos. La foto de este cierre quedará guardada en el historial.`))return;
  try{setBusy(true);await api(`/rest/v1/accounting_month_locks?month=eq.${encodeURIComponent(monthStart(m))}`,{method:'DELETE'});await loadMonthLocks();toast(`${monthLabel(m)} reabierto · cierre anterior conservado`);renderApp()}catch(e){toast('No se pudo reabrir el mes: '+(e?.message||'error'))}finally{setBusy(false)}
}
function blockLockedCandidateLinks(){document.querySelectorAll('[data-doc-link]').forEach(input=>{const x=(Array.isArray(invoices)?invoices:[]).find(i=>i.id===input.value);if(!x||!documentMonthLocked(x))return;input.checked=false;input.disabled=true;const meta=input.closest('.doc-candidate')?.querySelector('.doc-candidate-meta');if(meta&&!meta.textContent.includes('mes cerrado'))meta.append(' · 🔒 mes cerrado')})}
...
invoiceRow=function(x,actions=false){let html=previousInvoiceRowMonthLock(x,actions);if(!actions||!documentMonthLocked(x))return html;html=html.replace(/<button type="button" class="secondary invoice-edit"[^>]*>✏️ Editar<\/button>/g,'').replace(/<button type="button" class="secondary danger invoice-delete"[^>]*>🗑 Borrar<\/button>/g,'').replace(/<button type="button" class="secondary" data-unlink-document="[^"]+">↩ Desvincular<\/button>/g,'');return html.replace('<div class="invoice-actions">','<div class="doc-month-lock">🔒 Mes cerrado</div><div class="invoice-actions">')};
...
saveInvoice=async function(){const date=v('invDate'),m=lockMonthKey(date),existing=lockedEditTarget();if((m&&isMonthLocked(m))||documentMonthLocked(existing)){toast(`🔒 ${monthLabel(documentMonthLocked(existing)?lockMonthKey(existing.invoice_date):m)} está cerrado. Reábrelo antes de guardar.`);return}try{return await previousSaveInvoiceMonthLock.apply(this,arguments)}catch(e){if(/MONTH_LOCKED/i.test(String(e?.message||''))){toast('🔒 Ese mes está cerrado. Reábrelo antes de modificar documentos.');return}throw e}};
...
addMonthLockStyles();loadMonthLocks().then(()=>{if(session)renderApp()});
})();
