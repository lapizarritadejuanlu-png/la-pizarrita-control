...
function trashMonthKey(x){return String(x?.invoice_date||'').slice(0,7)}
function trashMonthLocked(x){return !!x&&typeof window.isAccountingMonthLocked==='function'&&window.isAccountingMonthLocked(trashMonthKey(x))}
...
async function moveDocumentToTrash(id){
  ...
  if(trashMonthLocked(x)){toast('🔒 Ese mes está cerrado. Reábrelo antes de enviar el documento a Papelera.');return}
  ...
  try{setBusy(true);const restored=await api('/rest/v1/rpc/trash_document',{method:'POST',headers:{Prefer:'return=representation'},body:{p_document_id:id}});await restorePricesAfterInvoiceTrash(restored);if(editingInvoiceId===id){editingInvoiceId=null;editingFilePath=null}toast('Documento movido a Papelera');await loadData()}catch(e){const m=e?.message||'';toast(/MONTH_LOCKED/i.test(m)?'🔒 Ese mes está cerrado. Reábrelo antes de mover documentos.':'No se pudo mover a Papelera: '+m)}finally{setBusy(false)}
}
async function restoreDeletedDocument(id){
  const x=deletedDocuments.find(i=>i.id===id);if(!x)return;
  if(trashMonthLocked(x)){toast('🔒 Ese mes está cerrado. Reábrelo antes de restaurar este documento.');return}
  try{setBusy(true);const ok=await api('/rest/v1/rpc/restore_document',{method:'POST',body:{p_document_id:id}});if(ok===false)throw new Error('No se pudo restaurar');toast('Documento restaurado');await loadData()}catch(e){const m=e?.message||'';if(/MONTH_LOCKED/i.test(m))toast('🔒 Ese mes está cerrado. Reábrelo antes de restaurar.');else if(/duplicate key|unique constraint|23505/i.test(m))toast('No se puede restaurar porque existe otro documento igual.');else toast('No se pudo restaurar: '+m)}finally{setBusy(false)}
}
async function deleteDeletedDocumentForever(id){
  const x=deletedDocuments.find(i=>i.id===id);if(!x)return;
  if(trashMonthLocked(x)){toast('🔒 Ese mes está cerrado. Reábrelo antes del borrado definitivo.');return}
...
saveInvoice=async function(){if(!editingInvoiceId){const t=v('invDocType')||'invoice',supplier=trashNorm(v('invSupplier')),number=trashNorm(v('invNumber'));if(number&&supplier){const dup=deletedDocuments.find(x=>(x.document_type||'invoice')===t&&trashNorm(x.supplier)===supplier&&trashNorm(x.invoice_number)===number);if(dup){toast(`Ese ${trashType(dup).toLowerCase()} está en Papelera. Restáuralo en vez de guardarlo otra vez.`);return}}}return previousSaveTrash.apply(this,arguments)};
