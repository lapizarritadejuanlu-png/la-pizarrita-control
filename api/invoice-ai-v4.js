...
Si algo no se ve claro usa null. No inventes. Usa punto decimal.`;
    ...
    if(isPdf){
      ...
      const r=await generateText({model:'google/gemini-2.5-flash-lite',messages:[{role:'user',content:[{type:'text',text:prompt},{type:'file',mediaType:'application/pdf',data:pdf,filename:name||'documento.pdf'}]}],maxOutputTokens:3800,abortSignal:AbortSignal.timeout(70000)});
      raw=r.text||'';
    }else{
      const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),70000);
      try{
        const r=await fetch('https://ai-gateway.vercel.sh/v1/responses',{method:'POST',signal:controller.signal,headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:'google/gemini-2.5-flash-lite',input:[{type:'message',role:'user',content:[{type:'input_text',text:prompt},{type:'input_image',image_url:dataUrl,detail:'high'}]}],max_output_tokens:3800})});
        const text=await r.text();let d={};try{d=JSON.parse(text)}catch{}
        if(!r.ok){console.error('Image AI v4 gateway',r.status,text.slice(0,300));return res.status(502).json({code:'AI_SERVICE_ERROR',error:'No se pudo leer el documento ahora mismo. Inténtalo otra vez.'})}
    ...
    const items=Array.isArray(x.items)?x.items.slice(0,100).map(i=>{
      ...
      if(calculatedNet!==null&&Number.isFinite(calculatedNet)){
        ...
        if(up===null){
          ...
          // Incoherencia fuerte: no convertir una posible lectura desplazada en alerta de precio.
          up=null;
        }
      }
  ...
  }catch(e){console.error('Invoice AI v4',e?.name||'',e?.message||'');return res.status(502).json({code:'AI_SERVICE_ERROR',error:'La IA ha tardado demasiado o no pudo leer la imagen. Inténtalo otra vez.'})}
};
