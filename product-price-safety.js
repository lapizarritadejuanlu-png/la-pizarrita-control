...
function psGroups(){
  ...
  for(const p of Array.isArray(products)?products:[]){
    const key=`${psText(p.name)}|${psUnit(p.unit)}`;
    if(!map.has(key))map.set(key,[]);
    map.get(key).push(p);
  }
  const nameSourceUnits=new Map();
  for(const p of Array.isArray(products)?products:[]){
    if(!p?.source_invoice_id)continue;
    const k=`${psText(p.name)}|${p.source_invoice_id}`;
    if(!nameSourceUnits.has(k))nameSourceUnits.set(k,new Set());
    nameSourceUnits.get(k).add(psUnit(p.unit));
  }
  return [...map.values()].map(rows=>{
    ...
    if(prev&&Number.isFinite(now)&&Number.isFinite(before)&&Math.abs(before)>.000001){pct=((now-before)/Math.abs(before))*100;trend=pct>.001?'up':pct<-.001?'down':'same'}
    else if(prev)trend='same';
    const review=rows.some(p=>p?.source_invoice_id&&(nameSourceUnits.get(`${psText(p.name)}|${p.source_invoice_id}`)?.size||0)>1);
    return{rows,latest,prev,pct,trend,review};
...
function psButton(label,id){
  const inv=psInvoice(id);if(!inv?.file_path)return'';
  return `<button type="button" class="secondary product-source-file" data-product-source="${esc(id)}">${label}</button>`;
}
function decorateProductCards(){
  if(route!=='products')return;
  const groups=psGroups(),cards=[...document.querySelectorAll('.product-smart-row')];
  const select=document.getElementById('productTrend');
  ...
  cards.forEach((card,i)=>{
    ...
    if(g.review){card.dataset.trend='review';const w=document.createElement('div');w.className='product-review-warning';w.textContent='⚠ Revisar: este mismo nombre aparece con unidades distintas dentro de un mismo documento. No se usa como subida/bajada hasta comprobarlo.';card.querySelector('.price-main')?.appendChild(w)}
    const ids=[g.latest?.source_invoice_id,g.prev?.source_invoice_id].filter(Boolean),same=ids.length===2&&ids[0]===ids[1];
    let html='';
...
const oldBind=bind;
bind=function(){oldBind();addStyles();decorateProductCards();decorateDashboardPriceAlerts()};
window.safeProductTrendGroups=psGroups;
window.safeProductTrendCounts=safeCounts;
addStyles();
